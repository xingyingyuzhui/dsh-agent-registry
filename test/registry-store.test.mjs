...
test('isolated minimal clones are rewritten from the official standard file', async () => {
  ...
  const files = {
    '/tmp/standard/agent.cordis.yml': 'name: @deepseek-ai/dsh-tool-bash\n',
  ...
  const presets = {
    async list() {
      return [
        { id: 'standard', path: '/tmp/standard/agent.cordis.yml' },
        { id: 'wa-template', path: '/tmp/wa-template/agent.cordis.yml' },
        { id: 'wa-test1', path: '/tmp/wa-test1/agent.cordis.yml' },
      ]
    },
  }
  const result = await rewriteIsolatedMinimalClones(presets, {
    async readFile(file) { return files[file] },
    async writeFile(file, text) { files[file] = text },
  })
  assert.deepEqual(result.repaired, ['wa-template', 'wa-test1'])
  assert.equal(files['/tmp/wa-test1/agent.cordis.yml'], files['/tmp/standard/agent.cordis.yml'])
})
...
test('refreshSeedFiles upgrades stock identity to the OpenClaw-style template', () => {
  ...
  assert.match(next['SOUL.md'], /# SOUL/)
  assert.doesNotMatch(next['SOUL.md'], /你是 test1/)
  assert.match(next['IDENTITY.md'], /Claw Agent/)
  assert.match(next['IDENTITY.md'], /未定/)
  assert.doesNotMatch(next['IDENTITY.md'], /名字：test1/)
  ...
  assert.equal(next['BOOTSTRAP.md'], undefined)
...
test('claw slugs and legacy purge', () => {
  ...
  assert.match(seed['BOOTSTRAP.md'], /想叫你什么/)
  assert.match(seed['BOOTSTRAP.md'], /该怎么称呼你/)
  assert.doesNotMatch(seed['SOUL.md'], /你是 demo/)
  assert.doesNotMatch(seed['IDENTITY.md'], /名字：demo/)
  assert.match(seed['IDENTITY.md'], /未定/)
})
