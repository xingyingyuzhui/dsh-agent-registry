...
const COPY = {
  zh: {
    ...
    templateHint: '之后新建的 Agent 默认套用这里的核心、记忆、模型、权限和技能。不是某一个已有 Agent 的设置。',
    templatePersonaHint: '这里改的是新建时落下的人设底稿。第一次对话仍按 BOOTSTRAP.md 问名字。',
    templateMemoryHint: '记忆写入审批和回顾是全套共用的默认，新建 Agent 也走这里。',
    templateModelHint: '新建会话还没选模型时用这个默认。选「跟随官方」则不单独钉。',
    templateSkillsHint: '新建 Agent 默认关掉的技能。之后仍可在该 Agent 的技能 tab 改。',
    templateResetBody: '恢复成套件自带的底稿。未保存的修改会丢掉。',
    templatePreset: '常驻模板',
    templateMcp: 'MCP 默认',
  ...
  en: {
    ...
    templateHint: 'New agents start from these Core, Memory, Model, Permissions, and Skills defaults. This is not a setting of any existing agent.',
    templatePersonaHint: 'These files are copied when an agent is created. The first chat still follows BOOTSTRAP.md for names.',
    templateMemoryHint: 'Write approval and review are suite-wide defaults. New agents use them too.',
    templateModelHint: 'Used for new sessions that have no model yet. Inherit keeps the official default.',
    templateSkillsHint: 'Skills turned off for newly created agents. Each agent can still change its own list later.',
    templateResetBody: 'Restore the shipped draft. Unsaved edits are discarded.',
...
function createTemplatePage(React, t, post, toast, subscribeLocale, ReactDOM) {
